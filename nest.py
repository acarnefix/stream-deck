...
print(CLIENT_ID)
REDIRECT_URI = "https://www.aaron.com"
CODE = "4/0AX4XfWj3qiFHzF4QoOjKizfRuT0zcvZWNtWEyoChcZT3bDgwDgFcM-zQoJwQuxSwBj0mZQ"
BASE_PARAMS = {
    "client_id": CLIENT_ID,
...
REFRESH_TOKEN = (
    "1//0froojBrGVKx4CgYIARAAGA8SNwF-L9IrIjG9Q_iqN7brnFHjPi5SI84cKJdMg7uHe3QKn8J4yox41WQ7iPY4rK2IQuTdvsX8cAw"
)
PROJECT_ID = "acfdd0be-f091-4d4c-9e06-994512e485cb"
DEVICE_NAME = (
    "enterprises/acfdd0be-f091-4d4c-9e06-994512e485cb/devices/AVPHwEsBciJEQX1SGnvFUA5xAJbKOabGm7LhKt7ZsbmWfmYoUMWH7RCvC"
    "4na1RalhEq96fIB7uxnPFFDKNlmhYQ9GQQ0cw"
)
...
def get_device_name():
    url_get_devices = f"{DEVICE_URL}/enterprises/{PROJECT_ID}/devices"
    response_json = requests.get(url_get_devices, headers=request_headers()).json()
    return response_json["devices"][0]["name"]


def get_current_temperature():
    url_get_device = f"{DEVICE_URL}/{DEVICE_NAME}"
    response_json = requests.get(url_get_device, headers=request_headers()).json()
...
def execute_command(command_name, params):
    url_set_mode = f"{DEVICE_URL}/{DEVICE_NAME}:executeCommand"
...
def set_new_temperature(increment):
    execute_command(
        ...
        params={"coolCelsius": get_current_temperature() + increment}
    )
